...
import { 
  ...
  Crown, Star, Heart, Palette, Grid, Zap
} from "lucide-react";
  ...
  const fetchLeaderboardAndUser = useCallback(async (isBackgroundRefresh = false) => {
    ...
    try {
      ...
      let mappedProfiles: ProfileUser[] = (profiles || []).map((p: any) => {
        ...
        return {
          id: p.id,
          full_name: (p.full_name || p.name || p.username || "User").replace(/<[^>]*>?/gm, "").trim(),
      ...
      if (userInList) {
        ...
        if (userProfile) {
          ...
          setCurrentUser({
            id: p.id,
            rank: 0,
            full_name: (p.full_name || p.name || p.username || "You").replace(/<[^>]*>?/gm, "").trim(),
            avatar_url: p.avatar_url || p.avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${p.id}`,
    ...
    <div className="min-h-screen bg-[#F6FAFF] flex flex-col md:flex-row tracking-tight font-sans pb-20 md:pb-0">
      ...
      <header className="md:hidden sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 bg-amber-50 border border-amber-200/80 px-3 py-1 rounded-full text-amber-700 font-extrabold text-xs">
          ...
          <span>{currentUser?.xp_points || 0} XP</span>
        </div>
      </header>
        ...
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          <aside className="relative w-72 bg-white h-full p-6 flex flex-col justify-between shadow-2xl z-10">
            ...
            <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-3">
              <img
                src={currentUser?.avatar_url || "https://api.dicebear.com/7.x/bottts/svg?seed=BlueBot"}
                alt="User Avatar"
                className="w-10 h-10 rounded-xl object-cover bg-blue-100"
              />
              <div className="overflow-hidden">
                <p className="text-sm font-black text-[#0F172A] truncate">{currentUser?.full_name || "Artist"}</p>
                <p className="text-xs font-bold text-blue-600">Level {Math.floor((currentUser?.xp_points || 0) / 100) + 1}</p>
              </div>
            </div>
          </aside>
        </div>
      ...
      <main className="flex-1 p-4 sm:p-6 max-w-4xl mx-auto w-full space-y-4 overflow-y-auto">

        {/* Action / Back Navigation */}
        <div className="flex items-center justify-between gap-2">
          <Link
            href="/dashboard"
            className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-2xl bg-white border border-slate-200 text-slate-700 font-black text-xs sm:text-sm hover:bg-slate-50 transition-all shadow-2xs shrink-0"
          >
          ...
          {currentUser && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-[#2563EB] hover:bg-blue-600 text-white rounded-2xl font-black text-xs sm:text-sm border-b-4 border-blue-800 active:border-b-0 active:translate-y-1 transition-all shadow-2xs shrink-0 cursor-pointer"
            >
        ...
        <div className="bg-white p-5 rounded-[2rem] border border-slate-200/80 shadow-2xs flex items-center justify-between relative overflow-hidden">
          <div className="space-y-1 pr-3">
            <h1 className="text-2xl font-black text-[#0F172A]">Leaderboard Standings</h1>
          ...
          <img
            src={mascotImageUrl}
            alt="Mascot"
            className="w-28 sm:w-36 h-auto object-contain shrink-0 drop-shadow-md"
            crossOrigin="anonymous"
          />
        </div>

        {loading ? (
          <div className="min-h-[220px] flex items-center justify-center bg-white rounded-[2rem] border border-slate-200/80 p-8 shadow-2xs">
          ...
          <>
            ...
            {leaderboardData.length > 0 && (
              <div className="pt-2 grid grid-cols-3 gap-2 sm:gap-4 items-end w-full">
                {/* RANK 2 */}
                {rank2 ? (
                  <div className="bg-white border border-slate-200 rounded-[2rem] p-2.5 sm:p-4 text-center relative flex flex-col items-center shadow-2xs">
                    ...
                    <img
                      ...
                      alt={rank2.full_name}
                      className="w-10 h-10 sm:w-16 sm:h-16 rounded-2xl object-cover border-2 border-slate-100 my-1 bg-blue-50"
                      crossOrigin="anonymous"
                    />
                ...
                {/* RANK 1 */}
                {rank1 ? (
                  <div className="bg-amber-50/80 border-2 border-amber-200 rounded-[2.2rem] p-3 sm:p-5 text-center relative flex flex-col items-center shadow-2xs">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-amber-400 border-2 border-white flex items-center justify-center font-black text-xs text-white shadow-2xs mb-1">
                      #1
                    </div>
                    <img
                      src={rank1.avatar_url}
                      alt={rank1.full_name}
                      className="w-12 h-12 sm:w-20 sm:h-20 rounded-2xl object-cover border-2 border-amber-200 my-1 bg-amber-100/50"
                      crossOrigin="anonymous"
                    />
            ...
            <div className="bg-white rounded-[2rem] border border-slate-200/80 p-4 sm:p-6 shadow-2xs">
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="flex items-center gap-2.5 min-w-0">
                            <img
                              src={user.avatar_url}
                              alt={user.full_name}
                              className="w-9 h-9 rounded-xl object-cover border border-slate-200 shrink-0 bg-blue-100"
                              crossOrigin="anonymous"
                            />
      ...
      {/* Mobile Bottom Navigation Bar (Same standard 5 icons as Practice Page) */}
